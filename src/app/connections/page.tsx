...
const MSG: Record<string, { text: string; tone: "ok" | "err" }> = {
  requested: {
    text: "Request sent. They’ll see it on their Connections page.",
    tone: "ok",
  },
  no_user: {
    text: "No account found with that email. Try inviting them by link instead.",
    tone: "err",
  },
  self: { text: "You can’t connect with yourself.", tone: "err" },
  exists: {
    text: "You’re already connected (or a request is pending).",
    tone: "err",
  },
};
...
export default async function ConnectionsPage({
  ...
  const awaitingMe = links.filter((l) => {
    ...
    const approverId =
      l.requestedBy === "facilitator" ? l.meditatorId : l.facilitatorId;
    ...
    <div className="flex flex-col gap-10">
      ...
      <section className="flex flex-col gap-3">
        ...
        <form
          ...
          className="flex flex-col gap-3 rounded-lg border border-hairline bg-paper-raised p-5"
        >
          <input
            type="email"
            name="email"
      ...
      <section className="flex flex-col gap-3">
        ...
        <form
          action={createInvite}
          className="flex flex-wrap items-center gap-4 rounded-lg border border-hairline bg-paper-raised p-5"
        >
          <div className="flex flex-wrap items-center gap-4 text-sm text-ink-soft">
            <label className="flex items-center gap-2">
              <input
            ...
            {invites.map((inv) => (
              ...
              >
                <div className="min-w-0 flex-1">
                  <p className="text-xs text-ink-faint">
                    Invite for a{" "}
                    {inv.intendedRole === "facilitator" ? "teacher" : "student"}
                  </p>
