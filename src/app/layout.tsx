...
export default async function RootLayout({
  ...
  return (
    <html lang="en" className={`h-full ${lora.variable} ${mulish.variable}`}>
          ...
          <nav className="mx-auto flex max-w-3xl items-center gap-6 px-5 py-4">
            ...
            {user ? (
              ...
              <div className="ml-auto flex items-center gap-5 text-sm">
                <Link
                  href="/practice"
                  className="breath btn-primary hidden shrink-0 rounded-lg px-4 py-2 font-bold sm:block"
                >
                  Start your sit
                </Link>
                <Link href="/login" className="text-ink-soft hover:text-link">
                  Sign in
                </Link>
              </div>
            )}
