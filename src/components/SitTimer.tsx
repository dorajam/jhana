...
export function SitTimer() {
    ...
    <div className="mt-6 flex flex-col items-center gap-8">
      <TimerRing
        ...
        label={
          phase === "done"
            ? "○"
            : formatClock(phase === "idle" ? totalSec : remaining)
        }
        dimmed={phase === "done"}
      />
 ...
 anchor and practising are optional intentions. */}
          <div className="mt-2 flex w-full max-w-3xl flex-col gap-4 rounded-xl border border-hairline bg-paper-raised px-6 py-5">
            <OptionRow label="Length" tone="text-cobalt">
              {PRESETS.map((m) => (
                <Chip
                  key={m}
            ...
            <OptionRow label="Anchor" tone="text-jade">
              {ANCHORS.map((a) => (
                <Chip
                  key={a}
                  selected={a === anchor}
            ...
            <OptionRow label="Practising" tone="text-saffron-hover">
              {THEMES.map((t) => (
                <Chip
                  key={t.value}
          ...
          <button
            type="button"
            onClick={start}
            className="breath btn-primary mt-3 rounded-lg px-10 py-3 text-lg font-bold"
          >
            Start timer
          </button>
        </>
...
function OptionRow({
  ...
  label: string;
  /** Row label colour, so the three groups read as distinct at a glance. */
  tone?: string;
  children: React.ReactNode;
}) {
  return (
    <div className="flex flex-col gap-1.5 sm:flex-row sm:items-center sm:gap-3">
      <p
        className={`shrink-0 text-xs font-semibold uppercase tracking-widest sm:w-24 ${tone}`}
      >
        {label}
      </p>
